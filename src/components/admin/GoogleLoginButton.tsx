import React from 'react';
import { classes, style } from 'typestyle';

// Style from https://codepen.io/mupkoo/pen/YgddgB
const buttonStyle = style({
  transition: 'background-color .3s, box-shadow .3s',
  padding: '12px 16px 12px 42px',
  border: 'none',
  borderRadius: '3px',
  boxShadow: '0 -1px 0 rgba(0, 0, 0, .04), 0 1px 1px rgba(0, 0, 0, .25)',
  color: 'var(--dark)',
  fontSize: '16px',
  fontWeight: '500',
  fontFamily:
    '-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Oxygen,Ubuntu,Cantarell,"Fira Sans","Droid Sans","Helvetica Neue",sans-serif',
  backgroundImage:
    'url(data:image/svg+xml;base64,)',
  backgroundColor: 'white',
  backgroundRepeat: 'no-repeat',
  backgroundPosition: '12px 11px',

  $nest: {
    '&:hover': {
      boxShadow: '0 -1px 0 rgba(0, 0, 0, .04), 0 2px 4px rgba(0, 0, 0, .25)',
    },

    '&:active': {
      backgroundColor: '#eeeeee',
    },

    '&:focus': {
      outline: 'none',
      boxShadow: '0 -1px 0 rgba(0, 0, 0, .04), 0 2px 4px rgba(0, 0, 0, .25), 0 0 0 3px #c8dafc',
    },

    '&:disabled': {
      filter: 'grayscale(100%)',
      backgroundColor: '#ebebeb',
      boxShadow: '0 -1px 0 rgba(0, 0, 0, .04), 0 1px 1px rgba(0, 0, 0, .25)',
      cursor: 'not-allowed',
    },
  },
});

export function GoogleLoginButton(props: React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return (
    // eslint-disable-next-line react/prop-types
    <button className={classes(buttonStyle, props.className)} {...props}>
      {props.children || 'Sign in with Google'}
    </button>
  );
}
